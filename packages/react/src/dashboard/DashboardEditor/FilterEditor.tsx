...
export function FilterEditor({
  ...
  const { tables, getDisplayName, isLoading, error } = useSchema();
  ...
  if (error) {
    return (
      <div style={{ padding: theme.spacing.lg, textAlign: 'center', color: theme.colors.error }}>
  ...
  const handleColumnChange = useCallback(
    (value: string) => {
      ...
      setFormState((prev) => {
        const autoLabel = prev.column ? toTitleCase(prev.column) : '';
        const isLabelAutoGenerated = prev.label === '' || prev.label === autoLabel;
        return {
          ...prev,
          column: value,
          type: suggestedType,
          label: isLabelAutoGenerated ? toTitleCase(value) : prev.label,
          dynamic: isDynamic,
